...
class PluginUpdaterPro {
	...
	public function __construct( $_api_url, $_plugin_file, $_api_data = null ) {
		...
		$this->failed_request_cache_key = WPACU_PLUGIN_ID . '_edd_sl_failed_http_' . md5( $this->api_url );
        ...
        // [WPACU Custom]
        if ( isset($_GET['wpacu_force_plugin_updater'])
            /* [Fallback - Old Query String] */ || isset($_GET['wpacu_clear_plugin_cached_info']) /* [/Fallback - Old Query String] */ ) {
            $this->force_plugin_updater();
        }
        // [/WPACU Custom]
    }
	...
	public function init() {
		add_filter( 'pre_set_site_transient_update_plugins', array( $this, 'pre_set_check_update') );
	 ...
	 */
	public function pre_set_check_update( $_transient_data ) {
     ...
     * @return void
     */
    public function force_reset_update_plugins_site_transient_for_wpacu() {
    ...
    /**
     * If the latest version is not detected, it will force fetching it and attempt to update the plugin
     * in case a newer version is indeed detected
     *
     * @return void
     */
    public function force_update() {
        $this->force_reset_update_plugins_site_transient_for_wpacu();

        require_once ABSPATH . 'wp-admin/includes/class-wp-upgrader.php';

        $plugin        = $this->name;

        $title         = sprintf( __( '%s Update', 'wp-asset-clean-up-pro' ), WPACU_PRO_PLUGIN_STORE_ITEM_NAME );
	...
	private function get_cache_key() {
		...
		return WPACU_PLUGIN_ID . '_edd_sl_' . md5( serialize( $string ) );
	}
     ...
     * @return void
     */
    public function clear_cache_from_edd() {
        $cache_key = $this->get_cache_key();
        delete_option($cache_key);
    }

    /*
     * This forces the plugin to re-download the data needed for plugin updates
     * This is useful when there are errors downloading the update, and we need to clear the cache
     */
    public function force_plugin_updater() {
        $this->clear_cache_from_edd();
